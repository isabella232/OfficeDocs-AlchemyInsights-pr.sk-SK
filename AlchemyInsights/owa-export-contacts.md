---
title: '8000020: Exportovanie kontaktov'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000020"
- "2003"
ms.assetid: ''
ms.openlocfilehash: d7dfdf30ec51f393cb47cb3fe89577923d4ac054579391f1fa9700bb9c3c6319
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: sk-SK
ms.lasthandoff: 08/05/2021
ms.locfileid: "53983288"
---
# <a name="export-contacts"></a>Exportovanie kontaktov

1. V dolnej časti stránky v Outlook na webe vyberte tlačidlo <img src='data:image/png;base64,' />
 **Ľudia**.
2. Na paneli s nástrojmi vyberte položku **Spravovať**  >  **export kontaktov**. 
3. Vyberte, či chcete exportovať všetky kontakty alebo iba kontakty z konkrétneho priečinka, a potom vyberte položku **Exportovať**.
4. Výberom položky Uložiť v dolnej časti stránky **uložte** súbor "contacts.csv" do predvoleného priečinka Stiahnuté súbory. Alebo vyberte šípku vedľa položky Uložiť **a vyberte** položku **Uložiť ako**.

Ďalšie informácie nájdete v téme [Exportovanie kontaktov z Outlook.](https://support.office.com/article/10f09abd-643c-4495-bb80-543714eca73f#ID0EAACAAA=Outlook_on_the_web)

