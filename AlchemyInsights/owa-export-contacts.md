---
title: '8000020: ako exportovať kontakty'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000020"
- "2003"
ms.assetid: ''
ms.openlocfilehash: c8cc1f36b9956de76a4a217a4baaf254dddd9643
ms.sourcegitcommit: 631cbb5f03e5371f0995e976536d24e9d13746c3
ms.translationtype: MT
ms.contentlocale: sk-SK
ms.lasthandoff: 04/22/2020
ms.locfileid: "43768524"
---
# <a name="export-contacts"></a>Exportovať kontakty

1. V dolnej časti stránky programu Outlook na webe vyberte <img src='data:image/png;base64,' />
 **Ľudí**.
2. Na paneli s nástrojmi vyberte položku **Spravovať** > **exportné kontakty**. 
3. Vyberte, či chcete exportovať všetky kontakty alebo iba kontakty z konkrétneho priečinka, a potom vyberte položku **Exportovať**.
4. V dolnej časti stránky vyberte **Uložiť** , ak chcete uložiť "Contacts. csv" do predvoleného priečinka na prevzatie. Prípadne vyberte šípku vedľa položky **Uložiť** a vyberte položku **Uložiť ako**.

Ďalšie informácie nájdete v téme [exportovanie kontaktov z programu Outlook](https://support.office.com/article/10f09abd-643c-4495-bb80-543714eca73f#ID0EAACAAA=Outlook_on_the_web).

